...
void
gui::update_screen_dimensions(gui::State *gui_state, uint32 new_window_width, uint32 new_window_height)
{
...
void
gui::update_mouse_button(gui::State *gui_state)
{
...
void
gui::update_mouse(gui::State *gui_state)
{
...
gui::Container *
gui::make_container(gui::State *gui_state, const char *title, v2 position)
{
    ...
    if (container) {
        ...
        if (
            input::is_mouse_in_bb(gui_state->input_state,
                container->position,
                container->position + v2(container->dimensions.x, container->title_bar_height)) &&
            input::is_mouse_button_now_down(gui_state->input_state, GLFW_MOUSE_BUTTON_LEFT)
        ) {
...
void
gui::draw_heading(gui::State *gui_state, const char *str, v4 color)
{
...
bool32
gui::draw_toggle(gui::State *gui_state, Container *container, const char *text, bool32 *toggle_state)
{
    ...
    draw_frame(gui_state, position, button_bottomright, TOGGLE_BUTTON_DEFAULT_BORDER, LIGHT_DARKEN_COLOR);
    draw_rect(gui_state, position + TOGGLE_BUTTON_DEFAULT_BORDER,
        button_dimensions - (TOGGLE_BUTTON_DEFAULT_BORDER * 2.0f), button_color);
    draw_text(gui_state, "body", text, text_position, LIGHT_TEXT_COLOR);
...
gui::draw_named_value(gui::State *gui_state, Container *container, const char *name_text, const char *value_text)
{
    ...
    draw_text(gui_state, "body-bold", name_text, position, LIGHT_TEXT_COLOR);

    v2 value_text_position = position + v2(NAMED_VALUE_NAME_WIDTH, 0.0f);
    draw_text(gui_state, "body", value_text, value_text_position, LIGHT_TEXT_COLOR);
}
...
bool32
gui::draw_button(gui::State *gui_state, Container *container, const char *text)
{
    bool32 is_pressed = false;
    ...
    draw_frame(gui_state, position, bottomright, BUTTON_DEFAULT_BORDER, MAIN_DARKEN_COLOR);
    draw_rect(gui_state, position + BUTTON_DEFAULT_BORDER,
...
gui::draw_console(gui::State *gui_state, char *console_input_text)
{
    ...
    // Draw console log
    {
        ...
        draw_rect(gui_state, v2(0.0f, 0.0f),
    ...
    // Draw console input
    {
        ...
        draw_rect(gui_state, console_input_position,
            v2(gui_state->window_dimensions.x, console_input_height),
            MAIN_DARKEN_COLOR);

        draw_text(gui_state, "body", console_input_text,
...
void
gui::request_cursor(gui::State *gui_state, GLFWcursor *cursor)
{
...
void
gui::set_cursor(gui::State *gui_state)
{
...
void
gui::draw_rect(gui::State *gui_state, v2 position, v2 dimensions, v4 color)
{
...
gui::draw_text_shadow(
    ...
    draw_text(gui_state, font_name, str, position + TEXT_SHADOW_OFFSET,
        v4(0.0f, 0.0f, 0.0f, color.a * 0.2f));
}


void
gui::draw_container(gui::State *gui_state, Container *container)
{
    draw_rect(gui_state,
        container->position,
    ...
    draw_text_shadow(gui_state, "body", container->title, text_position, LIGHT_TEXT_COLOR);
    draw_text(gui_state, "body", container->title, text_position, LIGHT_TEXT_COLOR);
